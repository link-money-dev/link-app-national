...
<template>
  <div class="page" >
    ...
    <div class="content">
      ...
      <card class="icard" padding="2px 8px" >
        <div class="card-content" slot="card-content">
          <v-text-field
            ...
            type="text"
            placeholder='BTC or CNY'
            primary
          ></v-text-field>
          <div @click="addallasset(asset_list)" class="addallasset">{{$t("AddAll")}}</div>
          <ul class="currency-ul">
            <li class="currency-li" v-for="currency in asset_list" :key="currency.code">
              <div class="currency">
                <div class="currency-left">
                  <div class="code-wrapper">
                    <span class="code">{{currency.code}}</span>
                    <span class="host">{{$t("Issueer")}}: {{currency.host}}</span>
                  </div>
                  <div class="address">
                    {{currency.issueer | shortaddress}}
                  </div>
                </div>
...
<script>
  ...
  export default {
    ...
    mounted(){
      this.url = null
      this.asset_list=this.origin_asset_list
    },
    methods: {
      ...mapActions({
        trust: 'trust',
      ...
      // search(){
      //   this.asset_list = this.origin_asset_list
      //   for(var i=0;i<this.origin_asset_list.length;i++)
      //   {
      //     if(this.origin_asset_list[i].code.toLowerCase().trim()==this.url.toLowerCase().trim())
      //     {
      //       this.asset_list.push(this.origin_asset_list[i]);
      ...
      addTrust(currency){
        ...
        if(this.native.balance - this.reserve > this.base_reserve){
          console.log(this.native.balance)
          console.log(this.reserve)
          console.log(this.base_reserve)
