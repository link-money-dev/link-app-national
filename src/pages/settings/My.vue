...
<template>
  <div class="page">
      ...
      <div class="content">
          <card padding="10px 0px"  class="infocard My_my_infocard">
          ...
          <card padding="0px 0px" margin="10px 0px" class="infocard My_my_tabbar">
            <div slot="card-content">
                <ul class="settings-ul">
                    <li class="settings-li" @click="redirect(item.name)" v-for="item in menus" :key="item.name">
                        <i class="material-icons vcenter tmp_info0">{{item.icon}}</i>
                        <span class="tmp_info">{{$t(item.title)}}</span>
